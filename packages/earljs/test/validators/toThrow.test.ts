...
describe('toThrow', () => {
  describe('not negated', () => {
    ...
    it('fails when exception with different error message was thrown', () => {
      const test = () =>
        earl(() => functionThatThrows()).toThrow('Critical error!') // error message mismatch

      expect(test).to.throw(
        'Expected to throw Error("Critical error!") but threw Error("Horrible error!")',
      )
    })
  })
  ...
  describe('negated', () => {
    ...
    it('fails when unexpected exception was thrown', () => {
      ...
      expect(test).to.throw(
        `Expected not to throw anything but threw Error("Horrible error!")`,
      )
    })

    it('fails when exception with different error message was thrown', () => {
      const test = () =>
        earl(() => functionThatThrows()).not.toThrow('Horrible error!') // error message match

      expect(test).to.throw(
        'Expected not to throw Error("Horrible error!") but threw Error("Horrible error!")',
      )
    })
  })
  ...
  describe('with error matcher', () => {
    ...
    it('throws when error messages doesnt match', () => {
      ...
      expect(test).to.throw(
        'Expected to throw HttpError but threw Error("500")',
      )
    })
  })
})
